...
ibov1 <- read.csv("ibov.csv")

# prepare data set "vale"
vale2 <- select(vale1, 1:2) # select columns
colnames(vale2)[1] <- "date" # change column name
colnames(vale2)[2] <- "vale" # change column name
vale2$date <- dmy(vale2$date) # set first column as date
vale2$vale <- as.numeric(gsub(",", ".", gsub("\\.", "", vale2$vale)))
...
dolar2$dolar <- as.numeric(gsub(",", ".", gsub("\\.", "", dolar2$dolar)))
...
iron2$iron <- as.numeric(gsub(",", ".", gsub("\\.", "", iron2$iron)))

# prepare data set "copper"
copper2 <- select(copper1, 1:2) # select columns
colnames(copper2)[1] <- "date" # change column name
colnames(copper2)[2] <- "copper" # change column name
copper2$date <- dmy(copper2$date) # set first column as date
copper2$copper <- as.numeric(gsub(",", ".", gsub("\\.", "", copper2$copper)))

# prepare data set "nickel"
nickel2 <- select(nickel1, 1:2) # select columns
colnames(nickel2)[1] <- "date" # change column name
colnames(nickel2)[2] <- "nickel" # change column name
nickel2$date <- dmy(nickel2$date) # set first column as date
nickel2$nickel <- as.numeric(gsub(",", ".", gsub("\\.", "", nickel2$nickel)))

# prepare data set "coal"
coal2 <- select(coal1, 1:2) # select columns
colnames(coal2)[1] <- "date" # change column name
colnames(coal2)[2] <- "coal" # change column name
coal2$date <- dmy(coal2$date) # set first column as date
coal2$coal <- as.numeric(gsub(",", ".", gsub("\\.", "", coal2$coal)))

# prepare data set "ibov"
ibov2 <- select(ibov1, 1:2) # select columns
colnames(ibov2)[1] <- "date" # change column name
colnames(ibov2)[2] <- "ibov" # change column name
ibov2$date <- dmy(ibov2$date) # set first column as date
ibov2$ibov <- as.numeric(gsub(",", ".", gsub("\\.", "", ibov2$ibov)))
...
last <- merge(fifth, ibov2, by = "date")

# simple linear model regression
simple <- lm(vale ~ dolar, data = last)
simple <- lm(vale ~ iron, data = last)
simple <- lm(vale ~ copper, data = last)
simple <- lm(vale ~ nickel, data = last)
simple <- lm(vale ~ coal, data = last)
summary(simple)

# multiple linear model regression
linear <- lm(vale ~ dolar + iron + copper + nickel + coal, data = last)
summary(linear)
summ(linear)
...
quant <- rq(vale ~ dolar, data = last, tau = 0.50)
quant <- rq(vale ~ iron, data = last, tau = 0.50)
quant <- rq(vale ~ copper, data = last, tau = 0.50)
quant <- rq(vale ~ nickel, data = last, tau = 0.50)
quant <- rq(vale ~ coal, data = last, tau = 0.50)
summ(quant)

# multiple quantile model regression
quant <- rq(vale ~ dolar + iron + copper + nickel + coal, data = last, 
            tau = 0.50)
...
last$residuals <- quant$residuals

# check if vale beated ibovespa or not
test <- filter(last, date <= "2019-10-30") # subset dataframe
...
test <- mutate(test, residualsgroup1 = cut2(residuals, g = 6))
